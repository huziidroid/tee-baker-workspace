...
import { MutationArgs, QueryArgs, QueryErrorResponse, QueryResponse } from './react-query-base.type';

export const useAppQuery = <TQueryData, TSelectData = TQueryData>(args: QueryArgs<TQueryData, TSelectData>) => {
  const { queryFn, queryKey, select, showLoading, options, refetchInterval, enabled = true } = args;

  const { startLoading, stopLoading } = useLoadingHandler();

  const { isLoading, ...query } = useQuery<AxiosResponse<QueryResponse<TQueryData>>, AxiosError<QueryErrorResponse>, TSelectData, QueryKey>({
    ...options,
    queryKey,
...
export const useAppMutation = <TData, TVariables>(args: MutationArgs<TData, TVariables>) => {
  const { onError, onMutate, onSettled, onSuccess, mutationFn, options, showLoading } = args;
